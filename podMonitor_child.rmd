...
if(month(startdate)==month(now())){
  ...
  summary_text<-glue("{counts$lastSent} pods have been sent and {counts$lastNotSent} have not")
#equating to an additional {0.5*counts$lastNotSent} patient hours spent in BRI ED.")
}else{
header_text<-glue("In {month(startdate, label = TRUE, abbr = FALSE)} the pod was working {percent_working}% of the time.")
summary_text<-glue("{counts$lastSent} pods were sent and {counts$lastNotSent} were not")
#, equating to an additional {0.5*counts$lastNotSent} patient hours spent in BRI ED.")
}
```
#### `r header_text`
